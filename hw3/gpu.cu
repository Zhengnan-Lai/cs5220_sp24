...
void init_simulation(particle_t* parts, int num_parts, double size) {
    ...
    part_thrds = 512;
    bin_thrds = 64;
    part_blks = (num_parts + part_thrds - 1) / part_thrds;
    bin_blks = (num_bins * num_bins + bin_thrds - 1) / bin_thrds;

    // Count the number of particles in each bin
    count_parts_gpu<<<part_blks, part_thrds>>>(parts, num_parts, part_id_gpu, bin_id_gpu);
    // Prefix the bins
    ...
    thrust::exclusive_scan(bin_ptr, bin_ptr + (num_bins * num_bins + 1), prefix_bin_ptr);
}
...
void simulate_one_step(particle_t* parts, int num_parts, double size) {
    ...
    insert_bins_gpu<<<part_blks, part_thrds>>>(parts, num_parts, part_id_gpu, bin_id_gpu, prefix_bin_id_gpu);

    // Compute forces
    compute_forces_bin_gpu<<<bin_blks, bin_thrds>>>(parts, num_parts, part_id_gpu, prefix_bin_id_gpu);

    // Move particles
    move_gpu<<<part_blks, part_thrds>>>(parts, num_parts, size, bin_id_gpu);
